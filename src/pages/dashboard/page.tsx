...
// ---------------------------------------------------------------------------
// API BASE constant removed in favour of unified API client.
// The `api` module reads `VITE_API_URL` and appends `/api` automatically.
...
export default function Dashboard() {
  ...
  const loadCustomerConfig = async () => {
    try {
      ...
      // Load configuration via unified API client.  This will throw
      // on non‑2xx responses, so no explicit res.ok check is needed.
  ...
  try {
    ...
    await api.post(`/customers/${customerId}/config`, payload);
